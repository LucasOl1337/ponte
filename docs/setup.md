...
`notebook` watches `notebook-omarchy` (`100.91.100.95`) with Sunshine and Moonlight over Tailscale. Pair once with `./ponte notebook pair` (PIN 7391). Confirm the notebook watching this PC with `./ponte notebook accept` (PIN 4826). Then `./ponte notebook view` opens a 1080p windowed Desktop stream.
