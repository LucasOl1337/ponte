...
test('a peer owned by the same tailnet user authorizes; others do not, and results are cached', async () => {
  ...
  const runner = async (command, args) => {
    ...
    if (address === '100.99.71.120') return whois(address, { User: OWNER });        // this PC
  ...
  const identity = createTailscaleIdentity({ runner, selfAddress: '100.99.71.120', env: {} });
...
test('auto-pairing is disabled when the owner cannot be resolved or PONTE_TAILSCALE_AUTO=0', async () => {
  const failing = createTailscaleIdentity({ runner: async () => { throw new Error('tailscale down'); }, selfAddress: '100.99.71.120', env: {} });
  await failing.ready;
  assert.equal(await failing.authorize('100.111.221.82'), false);
  const off = createTailscaleIdentity({ runner: async () => whois('100.99.71.120', { User: OWNER }), selfAddress: '100.99.71.120', env: { PONTE_TAILSCALE_AUTO: '0' } });
